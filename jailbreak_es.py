...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_path",type=str, default="/data/sprrty/LM/jailbreak/output/reformulated_queries_sp/2025-07-01 17:12:19.363190-model=gpt-3.5-turbo-n_restarts=5.json")
    ...
    for i_request, reform_dict in tqdm(enumerate(reformulated_queries)):
        ...
        for req in reformed_requests:
            if judge_rule_based_sp(req):
               reformed_request = req
               break


        print(f">>> Request #{i_request}: {raw_request}")
        if reformed_request is not None and judge_rule_based_sp(reformed_request):
    ...
    file_name = f'./output/jailbreak_v1_sp_output/{timestamp_start}-target_model={args.target_model}-reform_model={reform_model}-n_restarts={args.n_restarts}.json'

    if not os.path.exists('./output/jailbreak_v1_sp_output'):
        os.makedirs('./output/jailbreak_v1_sp_output')
